...
const InvoiceAddN = () => {
  ...
  const { data: contractRefsData } = useContactRef();
  const { data: prereceiptsData } = usePrereceipts();
  const { data: productCustomDescriptionData } = useProductCustomDescription(
    formData.contract_product
  );
  const { data: grcodeData } = useGRCode(formData.contract_product);
  ...
  const fetchContractData = async (value) => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/api/panel-fetch-contract-by-ref`,
        { contract_ref: value },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      throw new Error(
        error.response?.data?.message || "Failed to fetch contract data"
      );
    }
  };

  // UseQuery Hook
  const { data: ContractRefData, error } = useQuery({
  ...
  const handleSelectChange = useCallback(
    (field, value) => {
      ...
      //   if (selectedContract && selectedContract.contract_ref === value) {
      //     console.log(selectedContract.branch_short, "short");

      //     setFormData((prev) => ({
      //       ...prev,
      //       branch_short: selectedContract.branch_short,
      //       invoice_no: selectedContract.contract_no,
      //     }));
      //   }
      // }
      ...
      if (field === "contract_ref") {
        ...
        if (selectedContract) {
          ...
          console.log("Contract data is not yet available.");
        }
      }
  ...
  const {
    ...
    onSuccess: (data) => {
      ...
      if (!data.scheme) {
        console.log("Empty or null scheme data.");
      }
    },
    onError: (error) => {
              ...
              <div className="grid grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="bg-white"
                    onChange={(e) =>
                      handleInputChange("invoice_ref", e.target.value)
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <MemoizedSelect
                    ...
                    options={
                      prereceiptsData?.prereceipts?.map((product) => ({
                        value: product.prereceipts_name,
                        label: product.prereceipts_name,
                      })) || []
                    }
              ...
              <div className="grid grid-cols-4 gap-6">
                <div>
                  ...
                  <MemoizedSelect
                    ...
                    options={
                      lutDatas?.scheme?.map((scheme) => ({
                        value: lutDatas.scheme.scheme_description,
                        label: lutDatas.scheme.scheme_description,
                      })) || []
                    }
