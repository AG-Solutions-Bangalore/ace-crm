...
const InvoiceDocumentEdit = () => {
    ...
    const updateBranch = async ({ id, data }) => {
        ...
        const response = await fetch(
          `https://exportbiz.in/public/api/panel-update-invoice-document/${id}`,
          {
            method: "PUT",
            headers: {
      ...
      const {
        ...
        queryFn: async () => {
          const token = localStorage.getItem("token");
          const response = await fetch(
            `https://exportbiz.in/public/api/panel-fetch-invoice-document-by-id/${id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                     ...
                     >
                       ...
                       <SelectContent    className="bg-white h-56">
                        {vesselData?.vessel.map((item,key)=>(
                            <SelectItem key={key} value={item.vessel_name}>{item.vessel_name}</SelectItem>
                        ))}
                     ...
                     >
                       ...
                       <SelectContent    className="bg-white">
                        {shipperData?.shipper.map((item,key)=>(
                            <SelectItem key={key} value={item.shipper_name}>{item.shipper_name}</SelectItem>
                        ))}
                     ...
                     <div>
                       ...
                       <Input
                         ...
                         onChange={(e) => handleDecimalInputChange(e, "invoice_i_value_usd")}
                     ...
                     <div>
                       ...
                       <Input
                         ...
                         onChange={(e) => handleDecimalInputChange(e, "invoice_exch_rate")}
