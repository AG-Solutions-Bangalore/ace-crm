...
const EditContainerSize = ({containerId}) => {
    ...
    const fetchStateData = async () => {
      ...
      try {
        ...
        setFormData({
            ...
            containerSize_status: containerSizeData.containerSize_status || "Active",
        });
        setOriginalData({
            containerSize: containerSizeData.containerSize || "",
            containerSize_status: containerSizeData.containerSize_status || "Active",
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: error.response?.data?.message || "Failed to update container size",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
               ...
               <div className="grid gap-2">
                 <div className="grid gap-1">
                   ...
                   <div className="relative">
                     ...
                     />
                     {hasChanges && formData.containerSize !== originalData.containerSize && (
                       <div className="absolute right-2 top-1/2 -translate-y-1/2">
                         <RefreshCcw
                           className="h-4 w-4 text-blue-500 cursor-pointer hover:rotate-180 transition-all duration-300"
                           onClick={() => setFormData(prev => ({ ...prev, containerSize: originalData.containerSize }))}
                         />
                       </div>
                     )}
                   </div>
                 ...
                 <div className="grid gap-1">
                   <label htmlFor="containerSize_status" className="text-sm font-medium">
                     Status
                   </label>
                   <Select
                     value={formData.containerSize_status}
                     onValueChange={(value) =>
                       setFormData(prev => ({ ...prev, containerSize_status: value }))
                     }
                   >
