...
const CreateCosting = () => {
  ...
  useEffect(() => {
    const totalMaterialCost = consigneData.reduce((acc, item) => {
      const percentage = parseFloat(item.costingSub_percentage) || 0;
      const rmCost = parseFloat(item.costingSub_rm_cost) || 0;

      const materialCost = (percentage / 100) * rmCost;
      return acc + materialCost;
    }, 0);

    setCostingData((prev) => ({
      ...prev,
      costing_raw_material: totalMaterialCost.toFixed(2),
    }));
  }, [consigneData]);
